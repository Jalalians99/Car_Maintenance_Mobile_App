...
type EditCarScreenNavigationProp = StackNavigationProp<RootStackParamList, 'EditCar'>;
type EditCarScreenRouteProp = RouteProp<RootStackParamList, 'EditCar'>;

interface Props {
  navigation: EditCarScreenNavigationProp;
  route: EditCarScreenRouteProp;
}
...
const EditCarScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const loadCarData = async () => {
    try {
      const car = await DatabaseService.getCar(carId);
      ...
      if (car) {
        setFormData({
          ...
          vin: car.vin || '',
          color: car.color || '',
          firstMaintenance: car.firstMaintenance || '',
          firstOilChangeDate: car.firstOilChangeDate || '',
          mileageAtFirstOilChange: car.mileageAtFirstOilChange?.toString() || '',
          oilChangeInterval: car.oilChangeInterval?.toString() || '',
        });
      }
    } catch (error) {
  ...
  const handleUpdateCar = async () => {
    ...
    try {
      ...
      await DatabaseService.updateCar(carId, updates);
      ...
      setSnackbarMessage(error.message || 'Failed to update car');
